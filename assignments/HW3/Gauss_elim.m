...
count = 0; %initialize interchange counter for correct det sighn
...
for ir1=1:n-1
    ...
    if (ipivmax ~= ir1)
        ...
        if (verbose)
            ...
            disp(Amod(ord,:));
            count = count + 1; %for correct det sighn counts row interchanges
        end %if
    end %if
...
%Determinate eq 1.119 from Hoffman textbook
A_final = Amod(ord,:);
det = A_final(1,1);
for k = 2:n %loop through rows
    det = det*A_final(k,k);
end %for
%disp(count)

%Correct the Determinate sighn for row interchanges
if rem(count,2)==0
    %det=det so do nothing for even row interchanges
else
    det =det*-1; %flip the sighn
end %if
